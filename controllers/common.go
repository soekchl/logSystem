...
func (this *BaseController) auth() {
	...
	if len(arr) == 2 {
		...
		if userId > 0 {
			user := GetUserListForId(userId)
			if user != nil && password == libs.Md5([]byte(this.getClientIp()+"|"+user.Password+user.Salt)) {
				this.userId = user.Id
	...
	if this.userId == 0 && (this.controllerName != "main" ||
		(this.controllerName == "main" &&
			...
			//			this.actionName != "apiphonecode" &&
			//			this.actionName != "apiregister" &&
			//			this.actionName != "apiresetpasswd" &&
			//			this.actionName != "apipaynotice" &&
			//			this.actionName != "agreement" &&
			this.actionName != "resetpasswd")) {
...
func getFlowStr(flow int32) string {
	if flow > 1000 {
		return fmt.Sprintf("%.1f GB", float32(flow)/1000.0)
	}
	return fmt.Sprint(flow, " MB")
}
